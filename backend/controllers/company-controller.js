...
exports.create = (req, res) => {
    client.query('INSERT INTO companies (name, description, email, website, logo, contract_plan, contract_term, contract_creation_date) ' +
...
exports.readAll = (req, res) => {
    client.query('SELECT companies.id, companies.name AS "name", companies.logo, companies.website, ' +
            'companies.email, companies.description, ' +
            "to_char(companies.contract_creation_date, 'DD.MM.YYYY') AS " + '"contractCreationDate" , ' +
...
exports.delete = (req, res) => {
    ...
    client.query('DELETE FROM companies WHERE id = $1', [req.params.id])
        .then((result) => {
            res.status(200).json({
                message: 'Deleted'
            })
...
exports.update = (req, res) => {
    ...
    if (req.file !== undefined) {
        company.avatar = 'uploads/companies/' + req.file.filename
        client.query('SELECT logo FROM companies WHERE id = $1', [company.id])
            .then((result) => {
                fs.unlinkSync(result.rows[0].logo)
                client.query('UPDATE companies SET name = $1, email = $2, website = $3, ' +
    ...
    nodemailer.createTestAccount()
        .then((result) => {
            ...
            transporter.sendMail({
                    ...
                    html: 'This <i>message</i> was sent from <strong>Node js</strong> server. <br/>' +
